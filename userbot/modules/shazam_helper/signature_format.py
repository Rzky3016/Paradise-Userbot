...
class RawSignatureHeader(LittleEndianStructure):
    ...
    _fields_ = [
        ...
        ('crc32', c_uint32),
        # Total size of the message, minus the size of the current header
        # (which is 48 bytes)
        ('size_minus_header', c_uint32),
        ('magic2', c_uint32),  # Fixed 0x94119c00 - 00 9c 11 94
        ('void1', c_uint32 * 3),  # Void
        # A member of SampleRate (usually 3 for 16000 Hz), left-shifted by 27
        # (usually giving 0x18000000 - 00 00 00 18)
        ...
        # Calculated as ((15 << 19) + 0x40000) - 0x7c0000 or 00 00 7c 00 -
        # seems pretty constant, may be different in the "SigType.STREAMING"
        # mode
...
class FrequencyPeak:
    ...
    def get_amplitude_pcm(self) -> float:
        ...
        return sqrt(exp((self.peak_magnitude - 6144) / 1477.3)
                    * (1 << 17) / 2) / 1024
...
class DecodedMessage:
    ...
    frequency_band_to_sound_peaks: Dict[FrequencyBand,
                                        List[FrequencyPeak]] = None
    ...
    def decode_from_binary(cls, data: bytes):
        ...
        self.sample_rate_hz = int(
            SampleRate(
                header.shifted_sample_rate_id >> 27).name.strip('_'))
    ...
    def encode_to_binary(self) -> bytes:
        ...
        header.shifted_sample_rate_id = int(
            getattr(
                SampleRate, '_%s' %
                self.sample_rate_hz)) << 27
        ...
        for frequency_band, frequency_peaks in sorted(
            ...
            for frequency_peak in frequency_peaks:
                ...
                if frequency_peak.fft_pass_number - fft_pass_number >= 255:
                    ...
                    peaks_buf.write(
                        (frequency_peak.fft_pass_number).to_bytes(
                            4, 'little'))
                ...
                peaks_buf.write(
                    (frequency_peak.peak_magnitude).to_bytes(
                        2, 'little'))
                peaks_buf.write(
    ...
    def encode_to_uri(self) -> str:
        ...
        return DATA_URI_PREFIX + \
            b64encode(self.encode_to_binary()).decode('ascii')
